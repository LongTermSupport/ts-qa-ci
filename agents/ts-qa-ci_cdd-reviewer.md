...
classes onto a raw `<button>` in a component-definition file the default
no-ad-hoc-html model exempts); your job is to catch exactly that.
...
Confirm the project's base-primitive directories (its `uiDirs` — usually
`src/components/ui/`, and any `src/components/layout/`). The doctrine's Tier A
boundary is three always-on rules working together:

- **`no-ad-hoc-html`** — raw HTML belongs only in `uiDirs` (strictest in
...
- **Primitive dirs (`uiDirs`)** MAY own raw HTML and author class-string
  literals — those are the component's internals styling its own states. Not a
  violation.
- **Composing dirs** (`feature/`, `screens/`, `pages/`, providers, …) MAY NOT
  author raw HTML or class piles. A class string there — bare, in `cn()/clsx()`,
  in a ternary/array/object key, or in a same-file `const` — is styling imposed
  at a call site and is a violation, even when a permissive lint config lets it
  through.

`require-variant-resolver` is opt-in and NOT the boundary — do not cite it as the
reason a composing-dir pile is wrong; cite the boundary trio.
...
2. **Copied primitive DNA** — a raw `<button>`/`<div>` carrying a primitive's
   ...
   a Button cva string). Tell-tale: comments like `// was variant="default"`.
...
1. Scope the review (changed files in a PR: `git diff --name-only <base>...HEAD`;
   or a directory). Read the doctrine and confirm `uiDirs`.
2. For each composing-dir file, scan every `className` and every raw lowercase
   tag. Apply the test. Grep for primitive base strings copied outside their home
   file.
...
2. [ad-hoc-html] src/pages/…:48
   Raw <button className="rounded-md bg-primary px-4 py-2"> on a page surface.
