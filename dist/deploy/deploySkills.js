...
export async function deploySkills(options) {
    ...
    console.log(`ts-qa: skills — ${skillsResult.copied.length} written, ${skillsResult.unchanged.length} unchanged; ` +
    ...
    if (existsSync(settingsLocalPath)) {
        ...
        try {
            ...
            console.warn(`ts-qa: WARNING — could not parse ${settingsLocalPath} — skipping hooks-key advisory ` +
                `(${cause instanceof Error ? cause.message : String(cause)}).`);
        }
        if (settingsLocal !== undefined && "hooks" in settingsLocal) {
            console.warn(`ts-qa: WARNING — ${settingsLocalPath} contains a "hooks" key. ts-qa-ci will never write there; ` +
