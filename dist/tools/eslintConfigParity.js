...
const tool = {
    ...
    async run(ctx) {
        ...
        if (!rootConfigPath && existsSync(join(ctx.cwd, "eslint.config.ts"))) {
            return {
                exitClass: "failure",
                stdout: "ts-qa: eslint.config.ts cannot be verified by the SSoT parity check " +
        ...
        let defaultExport;
        try {
            const mod = (await import(pathToFileURL(rootConfigPath).href));
            defaultExport = mod.default;
        }
        catch (error) {
            return {
                exitClass: "failure",
                stdout: `ts-qa: project-root ${rootConfigPath.replace(ctx.cwd + "/", "")} failed to load, ` +
                    `so it cannot be proven in sync with ts-qa's config: ${error.message}\n`,
                stderr: "",
            };
        }
        ...
        return {
            exitClass: "failure",
            stdout: `ts-qa: project-root ${rel} does NOT delegate to ts-qa's resolved config.\n` +
                `\n` +
...
export default tool;
//# sourceMappingURL=eslintConfigParity.js.map