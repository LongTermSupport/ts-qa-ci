...
function rightmostName(node) {
    ...
    if (node.expression)
        return rightmostName(node.expression);
    // `React.HTMLAttributes`: MemberExpression(.property) / TSQualifiedName(.right).
    if (node.property)
        return rightmostName(node.property);
    if (node.right)
        return rightmostName(node.right);
    return undefined;
}
...
const rule = {
    meta: {
        type: "problem",
        docs: {
            description: "`className` may not appear as a member of any interface or object type. Components own their styling via variant props; never accept className.",
    ...
    create(context) {
        ...
        if (!pathIncludesAny(context.filename, scopeGlobs))
            return {};
        const bearingTypes = new Set(options.classNameBearingTypes ?? DEFAULT_BEARING_TYPES);
        const bearingSuffixes = options.classNameBearingSuffixes ?? DEFAULT_BEARING_SUFFIXES;
        function isClassNameBearing(name) {
            if (!name)
                return false;
            if (bearingTypes.has(name))
                return true;
            return bearingSuffixes.some((suffix) => name.endsWith(suffix));
        }
        function reportBearing(base, atNode) {
            const name = rightmostName(base);
            if (!isClassNameBearing(name))
                return;
            context.report({
                node: atNode,
