const rule = {
    meta: {
        type: 'problem',
        docs: {
            description: 'Disallow explicit type arguments on querySelector/querySelectorAll; use a runtime instanceof filter.',
        },
        schema: [],
        messages: {
            typed: '`{{name}}<T>(...)` is an unchecked cast — the runtime returns whatever matches the CSS selector, not necessarily a T. Filter with `instanceof` after, e.g. `Array.from(...).filter((el): el is HTMLElement => el instanceof HTMLElement)`.',
        },
    },
    create(context) {
        ...
        return {
            CallExpression(node) {
                ...
                if (args === undefined || args === null)
                    return;
                if (args.type !== 'TSTypeParameterInstantiation')
                    return;
