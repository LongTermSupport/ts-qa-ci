...
export function formatLlmSummary(result, cachePath) {
    ...
    lines.push(`  jq -r '.phases[].toolResults | to_entries[] | select(.value.exitClass != "clean") | .key' ${cachePath}`);
    lines.push(`  jq -r '.phases[].toolResults | to_entries[] | select(.value.exitClass != "clean") | .value.stderr' ${cachePath}`);
    return lines.join('\n');
}
/**
