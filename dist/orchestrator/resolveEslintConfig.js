...
import { buildNonAppSurfacesBlock, loadNonAppSurfaces, } from "./nonAppSurfaces.js";
...
export function findTierARuleOverrides(configEntries) {
    ...
    for (const entry of configEntries) {
        ...
        if (linterOptions?.reportUnusedDisableDirectives !== undefined &&
...
export async function resolveEslintConfig(projectRoot, platform, packageRoot, quiet = process.env.TSQA_JSON === "1") {
    ...
    const unsanctioned = touchedTierARules.filter((touched) => !exemptions.some((exemption) => exemption.ruleId === touched.ruleId &&
