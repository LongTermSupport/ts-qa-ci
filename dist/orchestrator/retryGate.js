...
export async function retryGate(tool, ctx) {
    ...
    for (;;) {
        ...
        if (result.exitClass === 'crash')
            return result; // caller aborts on crash, never retries
        // result.exitClass === 'failure'
        if (ctx.ci)
            return result; // CI: fail-fast unless --aggregate (handled by runPhase.ts)
...
export const RESTART_WARNING = 'RAN WITH RETRIES — re-run the whole pipeline to be sure everything is fine (a retried tool does not re-validate phases that already passed before the fix).';
