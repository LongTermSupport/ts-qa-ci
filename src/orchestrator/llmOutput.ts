...
export function llmCachePath(cwd: string): string {
  return join(cwd, 'node_modules', '.cache', 'ts-qa', 'llm', LLM_CACHE_FILENAME);
}

/** Persists the FULL PipelineResult to the stable cache file; returns its path. */
export function writeLlmCache(result: PipelineResult, cwd: string): string {
...
interface FailingTool {
  phase: number;
  tool: string;
  exitClass: ToolResult['exitClass'];
}

function collectFailingTools(phases: PhaseResult[]): FailingTool[] {
 ...
 */
export function formatLlmSummary(result: PipelineResult, cachePath: string): string {
  const lines: string[] = [];
  ...
  if (result.hasBeenRestarted) {
    lines.push('Note: a tool was retried — re-run the full pipeline to re-validate earlier phases.');
  }
  lines.push('');
 ...
 */
export function emitLlmOutput(result: PipelineResult, cwd: string): string {
