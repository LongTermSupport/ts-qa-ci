...
describe('resolveDisabledTools', () => {
  ...
  it('reads disabledTools from tsQaConfig/ts-qa.json', () => {
    const { disabled, sources } = resolveDisabledTools(proj({ disabledTools: ['playwright'] }));
  ...
  it('ignores a missing config file and still honours CLI skips', () => {
    expect([...resolveDisabledTools(proj(), ['knip']).disabled]).toEqual(['knip']);
  });
  ...
  it('throws on malformed JSON rather than silently ignoring it', () => {
    expect(() => resolveDisabledTools(proj('{ not valid json'))).toThrow(/parse/i);
  });

  it('treats an object without a disabledTools key as no-op', () => {
    expect([...resolveDisabledTools(proj({ somethingElse: true })).disabled]).toEqual([]);
  });
