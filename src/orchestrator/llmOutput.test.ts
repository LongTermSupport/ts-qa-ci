...
import { emitLlmOutput, formatLlmSummary, llmCachePath, writeLlmCache } from './llmOutput.js';
...
describe('llmOutput', () => {
  ...
  const failingResult: PipelineResult = {
    ...
    phases: [
      ...
      },
      {
        ...
        failed: true,
        toolResults: {
          eslintReport: { exitClass: 'failure', stdout: 'lint out', stderr: 'lint err' },
  ...
  describe('formatLlmSummary', () => {
    it('renders a failing result with verdict, phase table, and failing tool', () => {
      const summary = formatLlmSummary(failingResult, '/tmp/cache/last-run.json');

      expect(summary).toContain('Verdict: FAIL at phase 2');
      // Per-phase PASS/FAIL table with each tool's exitClass.
      expect(summary).toContain('0      PASS    supplyChain:clean oxlint:clean');
      expect(summary).toContain('2      FAIL    eslintReport:failure knip:clean');
    ...
    it('surfaces a retry note when the pipeline was restarted', () => {
      const restarted: PipelineResult = { ...cleanResult, hasBeenRestarted: true };
      expect(formatLlmSummary(restarted, '/p/last-run.json')).toContain('a tool was retried');
    });
  });
  ...
  describe('writeLlmCache', () => {
    it('persists the full PipelineResult to node_modules/.cache/ts-qa/llm/last-run.json', () => {
      ...
      expect(path.endsWith(join('node_modules', '.cache', 'ts-qa', 'llm', 'last-run.json'))).toBe(
        true
      );
      ...
      expect(parsed.phases[1]?.toolResults.eslintReport?.stderr).toBe('lint err');
    });
  });
