...
describe('resolveLlmOutputMode', () => {
  ...
  it('defaults to "auto" when the config omits llmOutput', () => {
    expect(resolveLlmOutputMode(proj({ disabledTools: ['playwright'] }))).toBe('auto');
  });
  ...
  it('throws on an invalid llmOutput value', () => {
    expect(() => resolveLlmOutputMode(proj({ llmOutput: 'sometimes' }))).toThrow(
      /"llmOutput" .* must be one of auto, always, never/
    );
    expect(() => resolveLlmOutputMode(proj({ llmOutput: true }))).toThrow(/must be one of/);
  });
});
...
describe('resolveLlm precedence', () => {
  it('CLI --llm/--no-llm wins over everything', () => {
    ...
    expect(resolveLlm(input({ cli: true, env: env({ TSQA_LLM: '0' }), mode: 'never' }))).toBe(true);
    // --no-llm beats config "always", TSQA_LLM=1 and an agent env.
    expect(
      resolveLlm(input({ cli: false, env: env({ TSQA_LLM: '1', CLAUDECODE: '1' }), mode: 'always' }))
    ).toBe(false);
  });

  it('an explicit --json suppresses auto-enablement (json owns stdout)', () => {
    expect(resolveLlm(input({ json: true, env: env({ CLAUDECODE: '1' }), mode: 'always' }))).toBe(
      false
    );
  });

  it('TSQA_LLM overrides config when no CLI flag is given', () => {
    expect(resolveLlm(input({ env: env({ TSQA_LLM: '1' }), mode: 'never' }))).toBe(true);
    expect(resolveLlm(input({ env: env({ TSQA_LLM: '0' }), mode: 'always' }))).toBe(false);
  });
  ...
  it('config always/never wins over auto-detect', () => {
    ...
    expect(resolveLlm(input({ env: env({ CLAUDECODE: '1' }), mode: never }))).toBe(false);
  });

  it('auto falls through to detectLlm', () => {
    expect(resolveLlm(input({ env: env({ CLAUDECODE: '1' }), mode: 'auto' }))).toBe(true);
