...
export const ALL_TSQA_DISABLEABLE_RULE_IDS: readonly string[] = [
...
export const DEFAULT_SURFACES: Record<string, SurfaceDefault> = {
...
/** A per-surface override in ts-qa.json: `false` disables the surface; a string
 * (a dir → `<dir>/**`, or a literal glob) or string[] re-points its globs; an
 * object additionally overrides the disabled-rule set for that surface. */
type SurfaceOverride =
  false | string | string[] | { globs?: string | string[]; rules?: string[] };

interface SurfacesConfig {
  surfaces?: true | Record<string, SurfaceOverride>;
}
...
function normaliseGlob(value: string): string {
  if (/[*?[\]{}]/.test(value) || /\.[a-z]+$/i.test(value)) return value;
  return `${value.replace(/\/+$/, "")}/**`;
}

function toGlobs(value: string | string[]): string[] {
...
export function loadSurfaces(
  ...
  let parsed: SurfacesConfig;
  try {
    parsed = JSON.parse(readFileSync(configPath, "utf-8")) as SurfacesConfig;
  } catch (cause) {
  ...
  const overrides: Record<string, SurfaceOverride> = raw === true ? {} : raw;
  ...
  for (const name of names) {
    ...
    const kind: SurfaceKind = def?.kind ?? "nonApp"; // custom names default to nonApp
...
export function surfaceIgnores(
...
export function surfaceOffBlocks(
  surfaces: ResolvedSurface[] | undefined,
): Array<{ files: string[]; rules: Record<string, "off"> }> {
  if (!surfaces) return [];
  return surfaces
