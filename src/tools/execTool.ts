 ...
 */
function findToolPackageDir(packageRoot: string, pkg: string): string {
 ...
 */
export function bundledBin(packageRoot: string, name: string): string {
  ...
  const binField = (manifest as { bin?: string | Record<string, string> }).bin;
