...
function setCompliant(): void {
  files.clear();
  files.set(`${CWD}/package.json`, JSON.stringify({ packageManager: 'pnpm@11.1.2' }));
...
describe('auditSupplyChain', () => {
  ...
  it('flags a missing packageManager', () => {
    ...
    expect(auditSupplyChain(CWD, config).join('\n')).toMatch(/packageManager.*not set/i);
  });

  it('flags a non-pnpm packageManager (pnpm required)', () => {
    setCompliant();
    files.set(`${CWD}/package.json`, JSON.stringify({ packageManager: 'npm@10.9.0' }));
    expect(auditSupplyChain(CWD, config).join('\n')).toMatch(/pnpm is required/i);
  });

  it('flags a missing minimumReleaseAge', () => {
    setCompliant();
    files.set(`${CWD}/pnpm-workspace.yaml`, 'dangerouslyAllowAllBuilds: false\nverifyDepsBeforeRun: error\n');
    expect(auditSupplyChain(CWD, config).join('\n')).toMatch(/minimumReleaseAge is not configured/i);
  });
  ...
  it('flags dangerouslyAllowAllBuilds: true', () => {
    ...
    expect(auditSupplyChain(CWD, config).join('\n')).toMatch(/dangerouslyAllowAllBuilds is true/i);
  });
...
describe('supplyChain tool', () => {
  ...
  it('reads a custom floor from tsQaConfig/ts-qa.json', async () => {
    ...
    files.set(`${CWD}/tsQaConfig/ts-qa.json`, JSON.stringify({ supplyChain: { minReleaseAgeMinutes: 20160 } }));
    const result = await tool.run(ctx);
    expect(result.exitClass).toBe('failure');
