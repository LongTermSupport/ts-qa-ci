...
test('runs through a symlinked bin path (realpath-based direct-invocation check)', () => {
  ...
  try {
    ...
    const res = spawnSync(process.execPath, [link, '--bogus-arg'], { encoding: 'utf-8' });
    assert.equal(res.status, 2, `expected main() to run and reject the arg; got status ${res.status}`);
